...
Ensuite on test si le pseudo demandé par le client est valide, ceci ce fait en envoyant un message avec la commande CONNECT et le pseudo désiré par le client au serveur. Le serveur test alors l'unicité du pseudonyme, si celui ci est valide : le serveur envoie le message avec la commande ACCEPT au client qui démarre alors l'application de dessin; sinon le serveur envoie le message DENY et le client doit proposer un autre pseudo au serveur.
...
\paragraph{}Lorsque le client qui à la main effectue des modifications alors un message SUBMIT est evnoyé au serveur avec le nouveau dessin SVG comme contenu.
