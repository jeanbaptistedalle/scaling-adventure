...
public class ClientNetwork extends Thread {
	...
	/**
	 * @fn getInstance
	 * @brief retourne l'instance du singleton. Fait appel au constructeur s'il
	 *        n'y a pas encore d'intance.
	...
	public boolean initIp(String serverIpS) {
		if (sock == null || !sock.isConnected()) {
			try {
				...
				// Logger.getLogger(ClientNetwork.class.getName()).log(Level.SEVERE,
				// null, ex);
				return false;
	...
	public boolean initRoom(String roomNum) {
		try {
			...
			// String my_room = "";
			// for (int start = 0, end =
			// msg.getContent().indexOf(Constant.SEPARATOR, 0); end != -1; start
			// = end + 1, end = msg.getContent().indexOf(Constant.SEPARATOR,
			// start)){
	 ...
	 * @param cmd
	 *            la commande de ce message
	 * @param content
	 *            la chaîne de caractères contenue dans ce message
	 */
	...
	/**
	 * @fn updateUsers
	 * @brief Actualise la liste des utilisateurs à partir d'une chaîne de
	 *        caractères
	 * @param users
	 *            la chaîne de tous les utilisateurs
	 */
	...
	private void updateUsers(String users) {
		...
		for (int start = 0, end = users.indexOf(Constant.SEPARATOR); end != -1; start = end + 1, end = users
				.indexOf(Constant.SEPARATOR, start)) {
	...
	/**
	 * @fn submitPicture
	 * @brief envoie au serveur une mise à jour de l'image si le client a la
	 *        main
	 * @param modif
	 *            la chaîne de caractères qui représente la nouvelle image
	 */
	...
	/**
	 * @fn getUsers
	 * @brief Accesseur de la liste de clients connectés à la même Room que ce
	 *        client
	 * @return this.clients
	 */
	...
	public void setHaveControl(final boolean have_control){
		this.have_control = have_control;
		if (MainFrame.getInstance().getMenu() != null) {
			MainFrame.getInstance().getMenu().repaint();
		}
	}
	
	public void setRequestedControl(final boolean rqst_ctrl){
		this.rqst_ctrl = rqst_ctrl;
		if (MainFrame.getInstance().getMenu() != null) {
			MainFrame.getInstance().getMenu().repaint();
		}
	}
